    ...
    if((*L).first != NULL){
        ...
        while(bantu != NULL){
            for(int i = 0;i<banyak;i++){
                if((bantu->kontainer.nim == 1) && (data[i].nim == 1)){
                    addlastK(data[i].nim, (*L).first);
                }
                if((data[i].nim % bantu->kontainer.nim == 0) && (bantu->kontainer.nim != 1)){
                    addlastK(data[i].nim, bantu);
                }
            }
            bantu = bantu->next;
        }
