    ...
    if(root != NULL){
        ...
        if(hapus != NULL){
            ...
            //jika memiliki lebih dari satu simpul anak (child)
                ...
                if(ketemu == 0 && strcmp(hapus->nama, c) == 0) ketemu = 1;
                if(ketemu == 1){
                    simpul *last = root->child;
                    while(last->sibling != root->child){
                        last = last->sibling;
        ...
        if(strcmp(root->nama, c) == 0){
            ...
            if(bantu != NULL){
                ...
                if(bantu->sibling == NULL){
                    if(strcmp(bantu->nama, c) == 0) hasil = bantu;
...
void OlahInput(int banyak, char str[100]){
    int i;
    char findtempat[100], tempat[100], victimc[100];
    int victim = 0, count = 0, iterasi1 = 0, iterasi2 = 0;
    for(i=0;i<strlen(str);i++){
        if(count == 0 && str[i] != '#'){
            ...
            tempat[iterasi1] = str[i];
            iterasi1++;
        }else if(count == 1 && str[i] == '#'){
            tempat[iterasi1] == '\0';
            count++;
        }else if(count == 2){
            victimc[iterasi2] = str[i];
            iterasi2++;
        }
    }
    victimc[iterasi2] = '\0';
