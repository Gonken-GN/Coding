...
void sort(int banyak, list *L){
    ...
    for(int i=0;i<banyak;i++){
        //pointer akan terus direset ke elemen pertama
        elemen* temprev = (*L).first;
        elemen* tunjuk = (*L).first;
        j=0;
        //perulangan untuk iterasi pointer tunjuk
        while((tunjuk->next != NULL) && (j < banyak-1)){
            //proses sorting diurutkan berdasarkan harga barang
            if(strcmp(metode, "H") == 0){
                //jika elemen pertama (L.first) lebih besar daripada elemen kedua (tunjuk->next)
                ...
                tunjuk = tunjuk->next;
                j++;
                //proses mensorting barang berdasarkan kode barang    
